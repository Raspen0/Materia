...
public class BlockCrusher extends BlockContainer {
    ...
    public Item getItemDropped(int par1, Random random, int par3){
		return Item.getItemFromBlock(MateriaBlocks.blockCrusherIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
			FMLNetworkHandler.openGui(player, mainRegistry.instance, mainRegistry.guiIdCrusher, world, x, y, z);
			if(world.isRemote){
				return true;
		}
	...
	public static void updateMateriaFurnaceBlockState(boolean active, World worldObj, int xCoord, int yCoord, int zCoord) {
		...
		if(active){
			worldObj.setBlock(xCoord, yCoord, zCoord, MateriaBlocks.blockCrusherActive);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, MateriaBlocks.blockCrusherIdle);
		}
